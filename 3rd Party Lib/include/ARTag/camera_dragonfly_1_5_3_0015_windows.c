//PGR Dragonfly wrapper for new driver install (1.5.3.0015) 
//- June 28/2006 by Luke Burkett at IIT/NRC
//
...
if(greybar_rgb==0)
   ...
   //RGB camera
	//!!!
   //flycapture_error = flycaptureStart( flycapture_context[cam_num], FLYCAPTURE_VIDEOMODE_640x480RGB, FLYCAPTURE_FRAMERATE_15 );
	flycapture_error = flycaptureStart( flycapture_context[cam_num], FLYCAPTURE_VIDEOMODE_ANY, FLYCAPTURE_FRAMERATE_15 );

/*
	UPGRADE NOTES:

  I'm not really sure why, but 'FLYCAPTURE_VIDEOMODE_640x480RGB' was throwing an 'INVALID_MODE' error.
  I replaced that flag with 'FLYCAPTURE_VIDEOMODE_ANY', and it simply captures at whatever the camera
  currently has.

  The documentation says that there are certain incompatiblities with WindowsXP SP2, but I don't know
  if this is an example of one.
*/
...
/*
  ...
  The new version of the function "flycaptureConvertImage" does not take an 'unsigned char' array
  as a destination any more - it requires a 'FlyCaptureImage' struct. All I did was build a temp
  one to store the data, then I simply copied the image information (data member 'pData') to the
  original destination 'rgb_cam_image'.
*/


FlyCaptureImage dest;
dest.iCols = 0;
dest.iRows = 0;
dest.pixelFormat = FLYCAPTURE_BGR;

dest.pData = new unsigned char [image.iRows * image.iCols * 3];

flycapture_error = flycaptureConvertImage(flycapture_context[cam_num], &image, &dest);
flycaptureErrorToString( flycapture_error ); 
/*if(flycapture_error!=NULL) 
{
	char message [512];
	sprintf(message, "%s", flycapture_error);
	MessageBox(0, message, "ConvertImage", NULL);
}*/

size_t size = image.iRows * image.iCols * 3;
memcpy(rgb_cam_image, dest.pData, size);
free(dest.pData);


#ifdef TIMER_LOADED
 get_time(&seconds,&now);   
 camera_dragonfly_convert8torgb_ms=now-begin;
#endif
